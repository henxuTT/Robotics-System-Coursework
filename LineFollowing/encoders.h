...
class ENCODERS_c {
  public:
    ...
    void record_start(){
      ...
      start_count_e1 = count_e1;
      start_time = micros();
    }    

    void record_end(){
      end_count_e0 = count_e0;
      end_count_e1 = count_e1;
      end_time = micros();
      distance_er = float(end_count_e0-start_count_e0) / CPR * 2 * PI * WHEEL_R;
      distance_el = float(end_count_e1-start_count_e1) / CPR * 2 * PI * WHEEL_R;      
      dtime = float(end_time-start_time);
      v_er =  distance_er / float(end_time-start_time) * 1000000;
      v_el =  distance_el / float(end_time-start_time) * 1000000;
       
      start_count_e0 = end_count_e0;
      start_count_e1 = end_count_e1;
      start_time = end_time;
    }
...
ISR( INT6_vect ) {
  ...
  // State:  new B   new A   old B   old A
    ...
    if( state_e0 == 1 || state_e0 == 7 || state_e0 == 8 || state_e0 == 14 ) {
      count_e0 = count_e0 - 1;
    } else if( state_e0 == 2 || state_e0 == 4 || state_e0 == 11 || state_e0 == 13 ) {
      count_e0 = count_e0 + 1;
    } else {
      Serial.println("ERROR: INVALID STATE_E0");
...
ISR( PCINT0_vect ) {
    ...
    // Complete this if statement as necessary.
    // Refer to the labsheet. 
    if( state_e1 == 1 || state_e1 == 7 || state_e1 == 8 || state_e1 == 14 ) {
      count_e1 = count_e1 - 1;
    } else if( state_e1 == 2 || state_e1 == 4 || state_e1 == 11 || state_e1 == 13 ) {
      count_e1 = count_e1 + 1;
    } else {
      Serial.println("ERROR: INVALID STATE_e1");
    } // Continue this if statement as necessary.
